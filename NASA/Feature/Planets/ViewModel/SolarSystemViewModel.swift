...
class SolarSystemViewModel: ObservableObject {
    ...
    private var fetchPlanetsUseCase: FetchPlanetsUseCase = FetchPlanetsUseCase()
...
//                self.planets = data
//                print(self.planets?.bodies?.count ?? 0)
//            }
//            .store(in: &subscriptions)
        planets = try? JSONDecoder().preview(type: Planets.self, from: planetsJsonString)
    }
    
    var sorted: [Body] {
        return planets?.bodies?.sorted {
            $0.perihelion ?? 0 < $1.perihelion ?? 0
        } ?? []
    }
    
}

let planetsJsonString = """
{
  "bodies": [
    {
      "id": "uranus",
